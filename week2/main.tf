...
resource "azurerm_storage_account" "appstore" {
  ...
  #llow_blob_public_access = true
}

resource "azurerm_storage_container" "data" {
  name                  = "data"
  storage_account_name  = "appstorehwi"
...
#uploading our IIS Configuration script as a blob to the Azure storage account
...
resource "azurerm_lb" "app-balancer" {
  name                = "app-balancer"
  location            = azurerm_resource_group.app_grp.location
  resource_group_name = azurerm_resource_group.app_grp.name
  sku                 = "Standard"
...
resource "azurerm_lb_backend_address_pool" "PoolA" {
  loadbalancer_id = azurerm_lb.app-balancer.id
  name            = "BackEndAddressPool"

  depends_on = [
    azurerm_lb.app-balancer
  ]
}
